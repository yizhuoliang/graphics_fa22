...
description = `<b>Notes for October 6</b>
               <p>
	       Triangle meshes`;
...
\`);
setDescription(description + '<p><img src=imgs/visibility.jpg width=470>');
`,
...
distorting a square rubber sheet.
<p>
Formally, each point [x,y,z] on the
mesh surface is a function of parameters (u,v),
where u varies from 0.0 to 1.0 from the left to
right edge of the rubber sheet,
and
v varies from 0.0 to 1.0 from the bottom to
...
\`);
setDescription(description + '<p><img src=imgs/uv_to_sphere.png width=470>' +
                             '<p><img src=imgs/two_spheres.png width=470>');
`,
...
via a special kind of triangle mesh -- a <b>triangle strip</b>.
<p>
...
and glue all the rows together to
create one triangle strip
representing
...
<blockquote><pre>
a<sub>0</sub>  a<sub>1</sub>  ...  a<sub>m-1</sub>  <font color=red>a<sub>m-1</sub>  b<sub>0</sub></font>  b<sub>0</sub>  b<sub>1</sub>  ...  b<sub>n-1</sub>
</pre></blockquote>
The vertices in <font color=red>red</font> are the extra ones that we added.
</blockquote>
\`);
setDescription(description);
`,
...
\`);
setDescription(description + '<p><img src=imgs/cylinders.png width=470>');
`,
...
(a torus centered at the origin with 1.0 as the radius of the ring).
If r is the radius of the tube, then:
<blockquote><pre>
...
The upper figure shows a parametric torus.
The lower figure shows a variety of shapes
created either as parametric surfaces,
or by glueing together parametric surfaces.
</blockquote>
\`);
setDescription(description + '<p><img src=imgs/parametric_torus.png width=302>' +
                             '<p><img src=imgs/four_shapes.png width=302>');
`,
...
surface normals:
<blockquote><pre>
P &larr; M &bull; P   // MULTIPLY ON THE LEFT BY M
N &larr; N &bull; M<sup>-1</sup> // MULTIPLY ON THE RIGHT BY M INVERSE
</pre></blockquote>
Implementing this entirely in Javascript might look like this:
<blockquote><pre>
P = matrixTransform(M,
...
In practice, only <code>matrixInverse()</code> needs to be
in Javascript. Transformation of <code>P</code> and <code>N</code>
can be done in the vertex shader.
</blockquote>
\`);
setDescription(description);
`,
...
<p>
Because each square is actually composed of two triangles,
the resulting cube mesh is really an assembly of 12 triangles, as
you can see in the figure on the lower right.
</blockquote>
\`);
setDescription(description + '<p><img src=imgs/cube.png width=468>' +
                             '<p><img src=imgs/triangles_to_cube.png width=468>');
`,
...
\`);
setDescription(description + '<p><img src=imgs/extrusion.jpg width=470>');
`,
...
<blockquote><pre>
face_normal = AB &#x2A2F; BC + BC &#x2A2F; CA + CA &#x2A2F; AB
</pre></blockquote>
where the cross product of two vectors
...
\`);
setDescription(description + '<p><img src=imgs/computing_normals.jpg width=470>');
`,
