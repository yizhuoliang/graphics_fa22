...
*/
      ...
      for (let i = 0; i < nu; i++) {
         ...
         for (let j = 0; j < nv; j++) {
            ...
            let b = vertices[i + 1][j + 1].slice(0, 3);
            let c = vertices[i][j + 1].slice(0, 3);
            let d = vertices[i][j].slice(0, 3);
            let b_a = subtract(b, a);
            let c_b = subtract(c, b);
            let d_c = subtract(d, c);
            let a_d = subtract(a, d);
